...
1. Build the scene(s) to your iOS (requires XCode) or Android device (needs to be in developer mode). Make sure to follow the [quickstart guide](https://developers.google.com/cardboard/develop/unity/quickstart).
    * Make sure for Android to have 'Edit > Project Settings > XR Plug-in Management > Android' to have "Initialize XR on Startup" unchecked, and to only have "Cardboard XR Plugin" and "OpenXR" checked.
